var OPENAI_API_KEY = "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";
...
function Send() {
    ...
    if (str_input == "") { // Báo l?i và thoát kh?i ch??ng trình n?u prompt r?ng
        ...
        txtMsg.focus(); // ??t con tr? so?n th?o vào h?p nh?p
        return;
    }
    ...
    txtOutput.value += "User: " + str_input
    ...
    oHttp.onreadystatechange = function () {
        ...
        if (oHttp.readyState === 4) {
            ...
            catch (ex) // N?u quá trình parse không thành công: 
            {
                ...
                txtOutput.value += "Error: " + ex.message // L?u thông báo l?i vào stream output.
            }

            if (oJson.error && oJson.error.message) // N?u có l?i x?y ra
            {
                txtOutput.value += "Error: " + oJson.error.message; // L?u thông ?i?p l?i vào k?t qu? ??u ra
            }
...
function SpeechToText() {
    ...
    speech_recognizer.onresult = function (event) {
        ...
        for (var i = event.resultIndex; i < event.results.length; i++) // Duy?t t? ??u ??n cu?i c?a m?ng k?t qu?
        {
            ...
            {
                // Truy?n toàn b? k?t qu? nh?n di?n vào txtMsg
